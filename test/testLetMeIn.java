...
public class testLetMeIn {
    public static void main(String [] args){
        ...
        String lsSQL = "SELECT * FROM Inv_Transfer_Master WHERE sTransNox LIKE 'PR01%' AND cTranStat = '0' ORDER BY sTransNox";
        ResultSet loRS = poGRider.executeQuery(lsSQL);
        
        if (MiscUtil.RecordCount(loRS) <= 0){
            System.out.println("No Record to confirm.");
            System.exit(0);
        }
        
        InvTransfer loTransfer = new InvTransfer(poGRider, poGRider.getBranchCode(), true);
        
        try {
            poGRider.beginTrans();
            while (loRS.next()){
                if (loTransfer.openTransaction(loRS.getString("sTransNox"))){
                    if (!loTransfer.closeTransaction(loRS.getString("sTransNox"))){
                        poGRider.rollbackTrans();
                        System.err.println("Unable po confirm: " + loRS.getString("sTransNox"));
                        System.exit(1);
                    }
                }
            }
